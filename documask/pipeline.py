...
def _get_detectors() -> tuple[RegexDetector, Optional[NerDetector], YoloDetector]:
    global _regex_detector, _ner_detector, _yolo_detector
    if _regex_detector is None:
        _regex_detector = RegexDetector()
    if _yolo_detector is None:
        _yolo_detector = YoloDetector()
    # NER: лениво + с защитой от падения (torch может быть сломан на Windows)
    if _ner_detector is None:
        try:
            _ner_detector = NerDetector()
        except Exception:
            _ner_detector = None  # оставляем None, pipeline продолжит без NER
    return _regex_detector, _ner_detector, _yolo_detector
...
def run_pipeline(
    ...
    """
    regex_detector, ner_detector, yolo_detector = _get_detectors()
